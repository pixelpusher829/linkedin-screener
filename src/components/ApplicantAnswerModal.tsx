...
const QUESTION_PRESETS = [
  {
    ...
    question: "Why are you currently looking for a new opportunity and leaving your previous position?",
  },
  {
    label: "Why this company?",
    question: "Why do you want to work at our company and what caught your attention about this role?",
  },
  {
    label: "Design systems experience",
    question: "Tell us about your experience building and scaling enterprise design systems and bridging design with engineering.",
  },
  {
    label: "Handling ambiguity",
    question: "How do you handle projects with unclear requirements or shifting product scopes?",
  },
  {
    label: "Greatest technical accomplishment",
    question: "Describe your greatest technical accomplishment or a challenging problem you solved from scratch.",
  },
  {
    label: "Salary & Location eligibility",
    question: "What are your salary expectations and what is your location / work authorization status?",
  },
  {
    label: "Frontend & testing approach",
    question: "What is your philosophy on responsive UI architecture, web performance, and testing?",
  },
];
...
export const ApplicantAnswerModal: React.FC<ApplicantAnswerModalProps> = ({
  ...
  const [selectedJobId, setSelectedJobId] = useState<string>(initialSelectedJob?.id || "general");
  const [tone, setTone] = useState<"standard" | "concise" | "storytelling" | "direct">("standard");
  ...
  const handleGenerateAnswer = async (customPrompt?: string) => {
    ...
    try {
      ...
      setAnswer("Error generating answer: " + (err.message || "Please check connection."));
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="relative w-full max-w-3xl bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl overflow-hidden my-8 text-slate-100 flex flex-col max-h-[92vh] animate-in fade-in zoom-in-95 duration-150">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">
                Paste any recruiter or job application question to generate authentic first-person answers from your context doc
              </p>
            </div>
          </div>
          <button
        ...
        <div className="px-6 py-2.5 bg-slate-900 border-b border-slate-800 flex flex-wrap items-center justify-between gap-3 text-xs">
          ...
          <div className="flex items-center gap-2 flex-1 min-w-[240px]">
            ...
            >
              <option value="general">General / Any Application Question</option>
              {jobs.map((j) => (
            ...
            <div className="mt-4 p-4 rounded-xl bg-slate-950 border border-slate-800 space-y-3 animate-in fade-in duration-200">
              ...
              {isLoading ? (
                <div className="py-8 flex flex-col items-center justify-center gap-2.5 text-slate-400 text-xs">
                  ...
                  <span>Extracting facts and crafting personalized response...</span>
                </div>
              ) : (
                <>
                  ...
                  <div className="pt-2 border-t border-slate-800/80 flex flex-wrap items-center gap-1.5">
                    <span className="text-[11px] text-slate-500 font-medium mr-1">Refine:</span>
                    <button
                    ...
                    <button
                      onClick={() => handleRefine("Emphasize design systems and UX craftsmanship")}
                      className="px-2 py-0.5 rounded text-[11px] bg-slate-900 hover:bg-slate-800 text-slate-300 border border-slate-800 cursor-pointer"
                    >
                      🎨 Emphasize Design Systems
                    </button>
                    <button
                      onClick={() => handleRefine("Highlight modern Svelte and React technical depth")}
                      className="px-2 py-0.5 rounded text-[11px] bg-slate-900 hover:bg-slate-800 text-slate-300 border border-slate-800 cursor-pointer"
                    >
                      ⚡ Deep Technical Depth
                    </button>
                    <button
                      onClick={() => handleRefine("Include the CRM modernization case study at TEC")}
                      className="px-2 py-0.5 rounded text-[11px] bg-slate-900 hover:bg-slate-800 text-slate-300 border border-slate-800 cursor-pointer"
                    >
